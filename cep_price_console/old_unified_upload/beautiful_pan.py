...
class PanedWindow(Tk_PanedWindow):
    ...
    def _on_release(self, event):
        ...
        callback_id1 = self._list_of_panes[handle_index+1].bind(
            "<Configure>", lambda event, handle_index=handle_index: self._on_configure_pane(handle_index), "+")
    ...
    def add(self, pane, **kwargs):
        ...
        if quantity_of_panes >= 2:
            ...
            handle = self.HANDLE_CLASS(self, handle_index, bg=self._color, height=self._height, width=self._width,
                                       cursor=self._cursor, disallow_dragging=self._disallow_dragging,
                                       on_click=self._on_click, image=self._image)
...
if __name__ == "__main__":
    ...
    # image = PhotoImage(data="R0lGODlhAwAYAPIAAEBAQGBgYICAgLu7u8zMzAAAAAAAAAAAACH5BAEAAAUALAAAAAADABgAAAMXCCFRI4OUS"
    #                         "RVzUNJp24sbt3hZWHQYOCUAOw==")

    # noinspection SpellCheckingInspection
    image = PhotoImage(data="R0lGODlhGAADAPIFAEBAQGBgYICAgLu7u8zMzAAAAAAAAAAAACH5BAEAAAUALAAAAAAYAAMAAAMaWBJQym61N2UZJ"
                            "Tisb96fpxGD4JBmgZ4lKyQAOw==")
