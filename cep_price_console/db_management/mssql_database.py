from cep_price_console.db_management.server_utils import mssql_base
from sqlalchemy import Column, DECIMAL, DateTime, Integer, Table, Unicode, text
from sqlalchemy.ext.declarative import DeferredReflection


class MSSQL_Database(DeferredReflection, mssql_base):
    __abstract__ = True


aging_definition = Table(
    'AGING DEFINITION', MSSQL_Database.metadata,
...
ar_history = Table(
    'ARHISTORY', MSSQL_Database.metadata,
...
branch = Table(
    'BRANCH', MSSQL_Database.metadata,
...
buy_line = Table(
    'BUYLINE', MSSQL_Database.metadata,
...
cust_category = Table(
    'CUSTCATEGORY', MSSQL_Database.metadata,
...
customer = Table(
    'CUSTOMER', MSSQL_Database.metadata,
...
division = Table(
    'DIVISION', MSSQL_Database.metadata,
...
inventory = Table(
    'INVENTORY', MSSQL_Database.metadata,
...
inventory_history = Table(
    'INVENTORYHISTORY', MSSQL_Database.metadata,
    Column('PRODUCTCODE', Unicode(100), nullable=False),
...
invoice_detail = Table(
    'INVOICEDETAIL', MSSQL_Database.metadata,
...
invoice_header = Table(
    'INVOICEHEADER', MSSQL_Database.metadata,
    Column('INVOICECODE', Unicode(100), nullable=False),
    Column('CUSTOMERCODE', Unicode(100)),
    Column('SALESMANCODE', Unicode(100)),
...
invoice_summary = Table(
    'INVOICESUMMARY', MSSQL_Database.metadata,
    Column('INVOICEDATE', DateTime),
    Column('ORDERDATE', DateTime),
    Column('FISCALPERIOD', Unicode(4)),
    Column('SJPERIOD', Unicode(4)),
...
invoice_variance = Table(
    'INVOICEVARIANCE', MSSQL_Database.metadata,
    Column('CUSTOMERID', Integer),
    Column('SHIPTOID', Integer),
    Column('SHIPVIAID', Integer),
    Column('BRANCHID', Integer),
...
price_group = Table(
    'PRICEGROUP', MSSQL_Database.metadata,
...
prod_line = Table(
    'PRODLINE', MSSQL_Database.metadata,
...
product = Table(
    'PRODUCT', MSSQL_Database.metadata,
    Column('PRODUCTCODE', Unicode(100), nullable=False),
...
purchase_order_detail = Table(
    'PURCHASEORDERDETAIL', MSSQL_Database.metadata,
...
purchase_order_header = Table(
    'PURCHASEORDERHEADER', MSSQL_Database.metadata,
    Column('POCODE', Unicode(100), nullable=False),
    Column('RECEIVINGCODE', Unicode(100), nullable=False),
    Column('VENDORCODE', Unicode(100)),
...
salesman = Table(
    'SALESMAN', MSSQL_Database.metadata,
...
ship_to = Table(
    'SHIPTO', MSSQL_Database.metadata,
...
ship_via = Table(
    'SHIPVIA', MSSQL_Database.metadata,
...
territory = Table(
    'TERRITORY', MSSQL_Database.metadata,
...
vm_category = Table(
    'VMCATEGORY', MSSQL_Database.metadata,
...
warehouse = Table(
    'WAREHOUSE', MSSQL_Database.metadata,
    Column('WAREHOUSECODE', Unicode(100), nullable=False),
...
webcat = Table(
    'WEBCAT', MSSQL_Database.metadata,
